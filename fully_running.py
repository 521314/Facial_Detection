...
Plot their error curves:
  python kfkd.py plot_learning_curves net-fully-specialists.pickle
And finally make predictions to submit to Kaggle:
  python kfkd.py predict net-fully-specialists.pickle
"""
...
def plot_weights(weights):
    """
    plot point in the images,e.g.center_eyes,left_eyes
    """
...
def fit():
    """
    train CNN and store it as a "pickle" format.
    """
    X, y = load2d()
    net.fit(X, y)
    with open('net_new.pickle', 'wb') as f:
        pickle.dump(net, f, -1)
...
def fit_specialists(fname_pretrain=None):
    ...
    train six specific models but start from the "previous" CNN.
    In other words, these six models go ahead from the beginning that previous
    models get.
    """
    if fname_pretrain:
    ...
    for setting in SPECIALIST_SETTINGS:
        ...
        file.write("Training model for columns {} for {} epochs".format(
            cols, model.max_epochs)+"\n")
        model.fit(X, y)
        specialists[cols] = model
    file.close()
    with open('net-fully-specialists.pickle', 'wb') as f:
        # this time we're persisting a dictionary with all models:
        pickle.dump(specialists, f, -1)


def predict(fname_specialists='net-fully-specialists.pickle'):
    """
    load test data and predict the result
    """
...
def plot_learning_curves(fname_specialists='net-fully-specialists.pickle'):
